...
(function(mod) {
  ...
  function Display(place, doc) {
    ...
    d.prevInput = "";
    // Set to true when a non-horizontal-scrolling line shared is
    // added. As an optimization, line shared aligning is skipped when
    // this is false.
  ...
  function updateDisplayInner(cm, visible, forced) {
    ...
    // There might have been a shared with a focused element that got
  ...
  function measureCharInner(cm, prepared, ch, bias) {
    ...
    } else { // If it is a shared, simply get the box for the whole shared.
  ...
  // Return true when the given mouse event happened in a shared
  ...
  // not interfere with, such as a scrollbar or shared.
  ...
  // Signals that the marker's shared changed, and surrounding layout
  ...
  function markText(doc, from, to, options, type) {
    ...
    if (marker.replacedWith) {
      // Showing up as a shared implies collapsed (shared replaces text)
      marker.collapsed = true;
      marker.widgetNode = elt("span", [marker.replacedWith], "CodeMirror-shared");
  ...
  // they are entirely covered by collapsed, non-shared span.
